...
<br>
            ...
            <h2 class="page-header" style="margin-left: 150px">查看我的收藏 <span class="badge"  >${totolCount }</span></h2>
            <div class="table-responsive"style="margin-top: 50px;margin-left: 150px;margin-right: 150px">
                <table class="table table-striped table-hover">
                    ...
                    <tr>
                        ...
                        <th><span class="glyphicon glyphicon-list"></span> <span class="visible-lg">收藏票务名</span></th>
                    ...
                    <tbody>
                    <c:forEach items="${spliteMyLovelist }" var="p" varStatus="status">
                    <tr>

                        <td class="article-title">${status.count }</td>
                        <td>${p.bookInfo.ticketName }</td>
                      



                        <td> <a href="/DeleteoffLove?pid=${p.pid}">取消收藏</a></td>
...
<div style="width: 380px; margin: 0 auto; margin-top: 50px;">
		<ul class="pagination" style="text-align: center; margin-top: 10px;">
			...
			<c:if test="${curPage!=totalPage }">
				<li>
					<a href="${pageContext.request.contextPath}/MyMarkServlet?curPage=${currentPage+1 }" aria-label="Next"> 
						<span aria-hidden="true">&raquo;</span>
					</a>
				</li>
			</c:if>
            ...
            $("table tbody tr td a").click(function(){
                ...
                if (event.srcElement.outerText == "删除")
                {
                    if(window.confirm("此操作不可逆，是否确认？"))
                    {
                    	 $.ajax({
                             type: "POST",
                             url: "/Semester_Demo/DeleteMark",
                             data: "mid=" + id,
                             cache: false, //不缓存此页面
                             success: function (data) {
                             	alert(data);
                             	window.location.href="/Semester_Demo/MyMarkServlet?curPage="+${curPage};
                             }
                         });
                    };
                };
